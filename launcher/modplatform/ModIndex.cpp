...
{
    return static_cast<ModLoaderType>(static_cast<std::uint32_t>(lhs) | static_cast<std::uint32_t>(rhs));
}
...
static const QList<ModLoaderType> loaderList = { NeoForge, Forge, Cauldron, LiteLoader, Quilt, Fabric, Babric, BTA, LegacyFabric, Ornithe, Rift,
                                                 Cleanroom, NilLoader, CursedFabric, FoxLoader, FishModLoader };
...
auto getModLoaderFromString(QString type) -> ModLoaderType
{
    ...
    if (type == "nilloader")
        return NilLoader;
    if (type == "cursed-fabric")
        return CursedFabric;
    if (type == "foxloader")
        return FoxLoader;
    if (type == "fishmodloader")
        return FishModLoader;
    return {};
}
